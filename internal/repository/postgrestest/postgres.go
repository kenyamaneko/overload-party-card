...
// テーブル追加時に helper 側を更新せずに済むよう、information_schema 参照で
// 横展開時の更新漏れバグを防ぐ。
...
func RunMain(m *testing.M, out **Postgres, opts ...Option) int {
	...
	if err != nil {
		slog.Error("postgrestest start failed", "error", err)
		os.Exit(1)
	}
	*out = pg
	...
	defer func() {
		if cerr := pg.Close(ctx); cerr != nil {
			slog.Error("postgrestest close failed", "error", cerr)
		}
	}()
...
// repoRoot は本ファイルの位置 (postgrestest パッケージは
// internal/repository/postgrestest 配下に置かれる規約) から go.mod を持つ
// ディレクトリを探索して返す。build tag やソースキャッシュに左右されないよう
// runtime.Caller を anchor とする。
