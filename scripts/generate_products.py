...
Outputs:
  ...
  - db/seed/products_seed.sql      (PostgreSQL UPSERT seed)
...
def generate_seed_sql(products, *, out_path):
    """Generate db/seed/products_seed.sql with UPSERT statements for products and initiatives."""
    ...
    for product in sorted_products:
        product_id = _sql_escape(product["product_id"])
        for initiative in product["initiatives"]:
            effect_json = _sql_escape(json.dumps(initiative["effect"], ensure_ascii=False)) + "::jsonb"
            initiative_rows.append(
                f"  ({_sql_escape(initiative['initiative_id'])}, {product_id}, "
